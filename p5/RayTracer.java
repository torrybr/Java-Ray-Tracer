...
public class RayTracer {
	...
	private static void setCamCoordinates(List<Double> eye, List<Double> look, List<Double> up, int d,
		...
		Ray raycompute = new Ray(vertices, faces, lightSources, ambientLight, kd, ks, ka, vn, faceNormals,3, sphereSources);
		...
		if (sphereModel && polyModel) {
			...
			if (polyModel) {
				...
				OutputPPM.writePPM(combineScene(sphereScene, sphereTvals));
			}
			...
			for (List<Double> sphere : sphereSources) {
				...
				for (int j = 0; j < res.get(1); j++) {
					...
					//System.out.println("Just Shot all rays through" + sphere.toString());
				}
	...
	private static void pixelRays(int i, int j, List<Integer> res, List<Double> bounds, int d, Vector3D EV, Vector3D WV,
    	...
    	List <Double> sph = new ArrayList<Double>();
		...
		if (!sphere.isEmpty()) {
			...
			Ray.raySphereIntersection(ray, pixpt, sphere,3,accum,refatt);
		} else {
